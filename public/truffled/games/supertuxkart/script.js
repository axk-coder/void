...
async function load_data() {
  ...
  await extract_tar(data_url, "/data", false);
  if (!FS.analyzePath("/data/supertuxkart.git").exists) {
    throw new Error("SuperTuxKart data archive did not mount /data/supertuxkart.git.");
  }
}
...
async function load_config() {
  ...
  if (!response.ok) {
    response = await fetch("config_example.json");
  }
...
async function start_game() {
  ...
  if (typeof globalThis.callMain !== "function") {
    throw new Error("Emscripten callMain() is not available.");
  }
  globalThis.callMain([]);
