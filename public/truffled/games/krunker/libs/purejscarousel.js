(function () {
  ...
  PureJSCarousel.prototype.build = function () {
    ...
    _.maxIndex =
      Math.ceil(_.slidesContainer.offsetWidth / _.carousel.offsetWidth / 3) - 1;
    ...
    _.btnPrev.innerHTML =
      '<span class="material-icons plusOrMinus2">navigate_before</span>';
    ...
    if (window.addEventListener)
      _.btnPrev.addEventListener("click", () => _.goToPrevSlide());
    else if (window.attachEvent)
      _.btnPrev.attachEvent("onclick", () => _.goToPrevSlide());
    ...
    _.btnNext.innerHTML =
      '<span class="material-icons plusOrMinus2">navigate_next</span>';
    ...
    if (window.addEventListener)
      _.btnNext.addEventListener("click", () => _.goToNextSlide());
    else if (window.attachEvent)
      _.btnNext.attachEvent("onclick", () => _.goToNextSlide());
    ...
    for (i = 0; i < _.slides.length; i++) {
      ...
      slideClone.className += " purejscarousel-slide-clone";
      _.slidesContainer
        .querySelectorAll(".purejscarousel-slide")
        [i].insertAdjacentElement("beforebegin", slideClone);
    }
  };
  ...
  PureJSCarousel.prototype.goToSlide = function (n, dir, trigger) {
    ...
    if ("transition" in document.body.style)
      this.slidesContainer.style.transition =
        "margin-left " + this.speed + "ms" + " linear " + this.delay + "ms";
    ...
    function scrollEnd() {
      ...
      for (i = 0; i < slidesCount; i++) {
        if (direction === "next")
          ...
          _.slidesContainer.children[0].insertAdjacentElement(
            "beforebegin",
            _.slidesContainer.lastElementChild,
          );
      }
      _.slidesContainer.style.marginLeft =
        -_.slidesContainer.offsetWidth / 3 + "px";
      _.setControl(true);
    }
  };
  ...
  PureJSCarousel.prototype.destroy = function () {
    ...
    if (this.isEnabled === 1) {
      ...
      for (i = 0; i < this.slides.length; i++) {
        ...
        this.slidesContainer.insertAdjacentElement(
          "beforebegin",
          this.slides[i],
        );
      }
